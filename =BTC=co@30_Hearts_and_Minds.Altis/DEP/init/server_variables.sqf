...
switch (dep_worldname) do {
    ...
    case "tanoa": {
		...
		if (isNil "dep_ambushes")           then { dep_ambushes         = 0; };
		if (isNil "dep_forest_patrols")     then { dep_forest_patrols   = 0; };
...
if (isNil "dep_logging")     		then { dep_logging   		= false; };          // Write debug messages to log file
...
if (isNil "dep_max_ai_loc")         then { dep_max_ai_loc       = 0; };             // Maximum AI per location
